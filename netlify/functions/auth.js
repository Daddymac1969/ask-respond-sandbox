...
const GOOGLE_SHEET_URL = process.env.GOOGLE_SHEET_URL || process.env.GOOGLE_SCRIPT_URL || "https://script.google.com/macros/s/AKfycbysOYqL1QYPovR1ilb4j0oUpXCv_ekh-2YQbezhdYVZw774DAgB3yoRDDZ3rOB0CKiN/exec";
...
exports.handler = async (event) => {
  ...
  if (codeMap.length === 0) {
    return { statusCode: 500, body: JSON.stringify({ error: "No access codes configured" }) };
  }
  try {
...
async function logLogin(codeLabel) {
  try {
    const params = new URLSearchParams({ question: "[LOGIN] " + codeLabel, response: "", source: "SANDBOX-LOGIN", meta: "", ts: new Date().toISOString() });
    return new Promise((resolve) => {
      https.get(GOOGLE_SHEET_URL + "?" + params.toString(), (res) => {
        if ((res.statusCode === 301 || res.statusCode === 302) && res.headers.location) {
          https.get(res.headers.location, () => resolve()).on("error", () => resolve());
        } else { resolve(); }
      }).on("error", () => resolve());
    });
  } catch(e) {}
